...
void ListComponent::Delete() {
    if (scrollEvent_ != -1)
        Systems::GetGameWindow().eventHandler.Unsubscribe(WindowEventType::MOUSE_SCROLL, scrollEvent_);
}
...
void ListComponent::Start() {
    scrollEvent_ = Systems::GetGameWindow().eventHandler.Subscribe(WindowEventType::MOUSE_SCROLL, [this](float delta) {
        if (!isActive)
            return;
