...
void RegisterDefaultSerializers() {
    ...
    Serializer::AddJSONSerializer<std::shared_ptr<Material>>([](Serializer::SerializationArgs& args, const nlohmann::json& j) {
        if (!j.is_string())
            return false;
        args.Return(Game::GetGameInstanceBase()->GetRenderer().GetMaterial(j));
        return true;
    });
