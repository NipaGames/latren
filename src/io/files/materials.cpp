#include <latren/io/files/materials.h>
...
std::vector<std::pair<std::string, std::shared_ptr<Material>>> ParseMaterials(const json& materialJson, int* invalidMaterials = nullptr) {
    ...
    if (materialJson.contains("texture")) {
        ...
        if (!textureJson.is_object() || !textureJson.contains("src") || !textureJson["src"].is_string())
            return ParsingException(invalidMaterials);
        else if (Game::GetGameInstanceBase()->GetResources().textureManager.HasLoaded(textureJson["src"]))
            m->SetTexture(Game::GetGameInstanceBase()->GetResources().textureManager.Get(textureJson["src"]));
        else
