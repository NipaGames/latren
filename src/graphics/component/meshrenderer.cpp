...
void MeshRenderer::Start() {
    if (!object.empty()) {
        for (const auto& mesh : Game::GetGameInstanceBase()->GetResources().modelManager[object].meshes) {
            if (copyMeshes) {
...
void MeshRenderer::Render(const glm::mat4& projectionMatrix, const glm::mat4& viewMatrix, const glm::vec3& viewPos, const Shader* shader, bool aabbDebug) const {
    ...
    if (aabbDebug) {
        ...
        aabbShader.SetUniform("viewPos", Game::GetGameInstanceBase()->GetRenderer().GetCamera().pos);
...
JSON_SERIALIZE_TYPES([](Serializer::SerializationArgs& args, const nlohmann::json& j) {
    ...
    if (mesh == nullptr)
        return false;
    mesh->material = Game::GetGameInstanceBase()->GetRenderer().GetMaterial(material);
    mesh->GenerateVAO();
    args.Return(mesh);
    return true;
}, std::shared_ptr<Mesh>);