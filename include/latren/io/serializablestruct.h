...
protected:
    ...
    LATREN_API void AddMember(const SerializableStructMemberData&);
    template <typename T>
    T MemberField(const std::string& name, T* addr, const T& val) {
        AddMember({ name, addr, &typeid(T), MetaType::NONE });
        return val;
    }
    LATREN_API void* AddNewline();
    LATREN_API void* AddComment(const std::string&);
public:
    LATREN_API const SerializableStructMemberData& GetMemberData(const std::string&);
    LATREN_API void* GetMemberAddress(const std::string&);
    template <typename T>
